...
Usage (from repo root or gawah-backend):
  cd gawah-backend && python scripts/seed_demo.py
  python scripts/seed_demo.py --replace   # wipe prior seed refs first
"""
...
# Allow `python scripts/seed_demo.py` from gawah-backend/
...
def main() -> int:
    ...
    if args.replace:
        _purge_seed(db)
        print("Purged prior seed rows.")

    statements = _build_statements(audio_dir)
    for stmt in statements:
        db.save_statement(stmt)
        print(f"  statement {stmt.ref_code}  status={stmt.status}  flags={len(stmt.inconsistency_flags)}")

    cluster = _build_cluster()
    db.save_cluster(cluster)
    print(f"  cluster   {cluster.id}  statements={cluster.statement_count}")

    for call in _build_calls():
        db.upsert_call(call)
        print(f"  call      {call['call_id']}  → {call['ref_code']}")
    ...
    print(f"  Store: {settings.local_db_path}")
